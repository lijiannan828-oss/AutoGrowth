...
### Execution 1: drama-processor-job-bbwz4 ❌ **失败**
- **开始时间**: 2025-11-23T08:36:51
- **完成时间**: 2025-11-23T09:08:59
- **持续时间**: 32分钟
- **Task Count**: 100
- **Succeeded Count**: 100 (所有 task 容器成功完成)
- **对应 Job**: WoakR4Uo4HaskBtxRvuC (KR064P01S01_헤이트 메리지)
- **实际结果**: 10 个成功，490 个失败（98% 失败率）

### Execution 2: drama-processor-job-bnk8w ✅ **成功**
- **开始时间**: 2025-11-23T08:37:19
- **完成时间**: 2025-11-23T08:58:37
- **持续时间**: 21分钟
- **Task Count**: 100
- **Succeeded Count**: 100 (所有 task 容器成功完成)
- **对应 Job**: EkTTCVMLEyT2thFnLaKm (US044P01S01_Runaway Prince's Secret Vacation)
- **实际结果**: 540 个全部成功（100% 成功率）
...
2. **为什么一个成功一个失败？**
   - **先启动的 job (bbwz4)** 可能先获得资源，但后续资源不足
   - **后启动的 job (bnk8w)** 可能在资源分配上更有利
   - 或者 **先启动的 job** 在处理过程中遇到资源竞争，导致大量失败
...
2. 设置并发限制（maxConcurrentDispatches=1）
3. 自动排队和调度
