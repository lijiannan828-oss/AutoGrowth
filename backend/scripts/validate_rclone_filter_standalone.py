#!/usr/bin/env python3
"""
独立的 rclone filter 验证脚本（不依赖项目模块）。
...
def escape_filter_component(component: str) -> str:
...
def normalize_relative_path(folder: str, drama_root: str) -> str:
...
def compile_filter_rules(include_folders: List[str], drama_root: str) -> List[str]:
    """Compile rclone filter rules from include folders."""
    rules: List[str] = []
    for folder in include_folders:
        if not folder:
            continue
        relative_path = normalize_relative_path(folder, drama_root)
        parts = [part for part in relative_path.split("/") if part]
        if not parts:
            rules.append("+ /**")
            continue
        escaped = "/".join(escape_filter_component(part) for part in parts)
...
def build_filter_file(
    ...
    rules = compile_filter_rules(include_folders, drama_root)
    if not rules:
        return None
...
class FilterValidator:
    ...
    def test_escape_component(self) -> None:
        """测试 escape_filter_component 函数"""
        print("=" * 80)
        print("测试 1: escape_filter_component 函数")
        print("=" * 80)

        test_cases = [
            ("normal_folder", "normal_folder", "普通文件夹名"),
            ("[Final]Episodes", "[[]Final[]]Episodes", "方括号（修复后）"),
            ("folder?", "folder\\?", "问号"),
            ("folder*", "folder\\*", "星号"),
            ("folder with spaces", "folder with spaces", "中间空格"),
            ("folder with trailing space ", "folder with trailing space ", "末尾空格（注意：当前可能被strip移除）"),
            ("folder[1]", "folder[[]1[]]", "方括号数字（修复后）"),
            ("folder?*", "folder\\?\\*", "多个特殊字符"),
            ("[Final] Episodes", "[[]Final[]] Episodes", "方括号+空格（修复后）"),
        ...
        for input_val, expected, description in test_cases:
            actual = escape_filter_component(input_val)
    ...
    def test_normalize_relative_path(self) -> None:
        """测试 normalize_relative_path 函数"""
        print("=" * 80)
        print("测试 2: normalize_relative_path 函数")
        print("=" * 80)
        ...
        for full_path, root, expected, description in test_cases:
            actual = normalize_relative_path(full_path, root)
            passed = actual == expected
    ...
    def test_compile_filter_rules(self) -> None:
        """测试 compile_filter_rules 函数"""
        print("=" * 80)
        print("测试 3: compile_filter_rules 函数")
        print("=" * 80)
        ...
        for root, include_folders, expected_rules, description in test_cases:
            actual_rules = compile_filter_rules(include_folders, root)
    ...
    def test_build_filter_file(self) -> None:
        """测试 build_filter_file 函数"""
        print("=" * 80)
        print("测试 4: build_filter_file 函数（生成完整 filter 文件）")
        print("=" * 80)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for root, include_folders, description in test_cases:
                ...
                filter_file = build_filter_file(include_folders, root, temp_dir)
    ...
    def test_complex_scenarios(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for scenario in scenarios:
                ...
                rules = compile_filter_rules(scenario["include_folders"], scenario["drama_root"])
                print("生成的规则:")
                for i, rule in enumerate(rules, 1):
                    print(f"  {i}. {rule}")

                filter_file = build_filter_file(
    ...
    def identify_potential_issues(self) -> None:
        ...
        issues = [
            {
                "issue": "路径末尾空格可能被移除",
                "location": "normalize_relative_path 函数中的 strip('/')",
            ...
            {
                "issue": "转义字符覆盖",
                "location": "escape_filter_component 函数",
    ...
    def run_all_tests(self) -> None:
        ...
        print(
            ...
            "\n   示例命令：rclone ls my-drive:\"KR Programs/KR001_Drama\" --filter-from transfer.filter"
        )
